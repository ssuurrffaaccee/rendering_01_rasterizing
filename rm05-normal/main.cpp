...
Color ray_marching(const Ray& ray, SignedDistanceField* sdf,
  ...
  for (int i = 0; i < max_marching_step; i++) {
    ...
    if (distance <= MIN_DISTANCE) {
      return calculate_normal(position,sdf);
    }
    t = t + distance;
  }
...
int main() {
  ...
  auto sphere0 = make_sphere(Vec3{-0.3f, 0.0f, 0.0f}, 1.0f);
  auto sphere1 = make_sphere(Vec3{0.3f, 0.0f, 0.0f}, 1.0f);
  auto sphere2 = make_sphere(Vec3{0.0f, 0.3f, 0.0f}, 1.0f);
  auto sphere3 = make_sphere(Vec3{0.0f, -0.3f, 0.0f}, 1.0f);
  auto merged0 = make_union(sphere0, sphere1);
  auto merged1 = make_union(sphere2, sphere3);
  auto inter = make_intersection(merged0, merged1);
  ...
  while (!ray_generator.empty()) {
    ...
    Color color = ray_marching(ray, inter.get(), 10);
