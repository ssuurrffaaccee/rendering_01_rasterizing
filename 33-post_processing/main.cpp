...
int main() {
  try {
    ...
    lights.push_back(make_point_light(world_centor + Vec3{0.0f, 4.0f, 0.0f},
                                      Color{1.0f, 1.0f, 1.0f}));
    {
      ...
      auto uv_cube_instance1 =
          Instance{"uv_cube", Vec3{0.0f, -3.0f, 0.0f}, Vec3{50.0f, 1.0f, 50.0f},
                   eye<float, 3>(), false};
      uv_blinn_phong_renader_homoposition_uv_norm(renderer.get(), camera, image,
                                                  model_pool, uv_cube_instance1,
                                                  lights, scale_figure_texture);
    }
    {
      auto figure_texture =
      ...
      auto uv_cube_instance =
          Instance{"uv_cube", Vec3{0.0f, 1.0f, -4.0f}, Vec3{3.0f, 3.0f, 3.0f},
                   eye<float, 3>(), false};
      uv_blinn_phong_renader_homoposition_uv_norm(renderer.get(), camera, image,
                                                  model_pool, uv_cube_instance,
                                                  lights, figure_texture);
    }
    image.dump("xxxx");
    {
      SSAADepthImage post_processed_image{width, height, SSAA};
      simple_post_processing(renderer.get(), post_processed_image, model_pool,
                             image, "inversion_post_processing");
      post_processed_image.dump("inversion");
    }
    {
      SSAADepthImage post_processed_image{width, height, SSAA};
      simple_post_processing(renderer.get(), post_processed_image, model_pool,
                             image, "grayscale_post_processing");
      post_processed_image.dump("grayscale");
    }
    {
      SSAADepthImage post_processed_image{width, height, SSAA};
      kernel_post_processing(renderer.get(), post_processed_image, model_pool,
                             image, sharpen_kernel);
      post_processed_image.dump("sharpen");
    }
    {
      SSAADepthImage post_processed_image{width, height, SSAA};
      kernel_post_processing(renderer.get(), post_processed_image, model_pool,
                             image, blur_kernel);
      post_processed_image.dump("blur");
    }
    {
      SSAADepthImage post_processed_image{width, height, SSAA};
      kernel_post_processing(renderer.get(), post_processed_image, model_pool,
                             image, edge_detection_kernel);
      post_processed_image.dump("edge_detection");
    }
  } catch (MyExceptoin &e) {
    std::cout << e.what() << "\n";
  }
