...
int main() {
  try {
    ...
    camera_config.position_ = Vec3{0.0f, 0.0f, 12.0f};
    ...
    // lights.push_back(make_pbr_point_light(world_centor +
    // Vec3{4.0f, 8.0f, 4.0f},
      ...
      for (int col = 0; col < columns; ++col) {
        ...
        auto uv_sphere_instance =
            Instance{"uv_sphere", translate, Vec3{1.0f, 1.0f, 1.0f},
                     eye<float, 3>(), false};
        pbr_directed_light_uv_deferred_geometry_rendering_homoposition_uv_norm(
            renderer.get(), camera, g_buffer_image, model_pool,
            uv_sphere_instance, albedo, metallic, roughness, ao);
      }
    }
