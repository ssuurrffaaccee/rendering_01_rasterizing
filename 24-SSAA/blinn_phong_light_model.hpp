...
public:
  ...
  process(const FragmentStageData &frag) {
...
class TextureBlinnPhongFragmentShader : public FragmentShader {
public:
  TextureBlinnPhongFragmentShader() {}
  std::tuple<Vec<int, 2>, Color, float>
  process(const FragmentStageData &frag) {
    float depth = frag.second[2];
    Color final_color = Color{0.0f, 0.0f, 0.0f};
    std::vector<std::shared_ptr<Light>> &lights =
        *(std::vector<std::shared_ptr<Light>> *)get_global_data(0);
    Vec3 &camera_world_position = *(Vec3 *)get_global_data(1);
    Texture &texture = *(Texture *)get_global_data(2);
...
REGISTER_SHADER(vertex, VertexShader, blinn_phong, BlinnPhongVertexShader)
REGISTER_SHADER(fragment, FragmentShader, blinn_phong, BlinnPhongFragmentShader)
REGISTER_SHADER(fragment, FragmentShader, uv_blinn_phong, TextureBlinnPhongFragmentShader)