...
int main() {
  try {
    ...
    //       renderer.get(), camera, image, model_pool, sphere_instance, lights);
    // }
    {
      auto uv_sphere_instance =
          Instance{"uv_sphere", Vec3{0.0f, 0.0f, 0.0f}, Vec3{4.0f, 4.0f, 4.0f},
                   eye<float, 3>(), false};
      ...
      uv_blinn_phong_renader_homoposition_uv_norm(
          renderer.get(), camera, image, model_pool, uv_sphere_instance, lights,
          texture);
    }
    // {
    ...
    //   auto uv_cube_instance = Instance{"uv_cube", Vec3{1.9f, 0.0f, 0.0f},
    //                                    Vec3{1.0f, 1.0f, 1.0f}, rotate, false};
