...
void bloom_gauss_blur(
    Renderer *renderer,SSAADepthImage &image,
    const ModelPool &model_pool,SSAADepthImage &bright_image,bool is_horizontal) {
  auto &shader_keeper = ShaderKeeper::get();
...
void bloom_combine(
    Renderer *renderer,SSAADepthImage &image,
    const ModelPool &model_pool,SSAADepthImage &color_image,SSAADepthImage &bright_image,float exposure) {
  auto &shader_keeper = ShaderKeeper::get();
...
int main() {
  try {
    ...
    camera_config.position_ = Vec3{0.0f, 10.0f, 10.0f};
    ...
    // lights.push_back(make_ambient_light(Color{1.0f, 1.0f, 1.0f}));
    std::vector<std::pair<Vec3, Color>> bloom_cube_positions_and_colors;
    std::vector<std::pair<Instance, std::shared_ptr<Light>>>
        bloom_cube_instances_and_point_lights;
    bloom_cube_positions_and_colors.push_back(
        std::make_pair(Vec3{0.0f, 4.0f, -8.0f}, Color{0.0f, 0.0f, 6.0f}));
    bloom_cube_positions_and_colors.push_back(
        std::make_pair(Vec3{-5.0f, 0.0f, 0.0f}, Color{0.0f, 6.0f, 0.0f}));
    bloom_cube_positions_and_colors.push_back(
        std::make_pair(Vec3{5.0f, 0.0f, 0.0f}, Color{6.0f, 0.0f, 0.0f}));
    for (auto &position_and_color : bloom_cube_positions_and_colors) {
      auto point_light =
          // make_point_light(position_and_color.first,
          // position_and_color.second);
          make_distance_point_light(position_and_color.first,
                                    position_and_color.second, 1.0f, 0.09f,
                                    0.032f);
      lights.push_back(point_light);
      bloom_cube_instances_and_point_lights.push_back(std::make_pair(
          Instance{"uv_bloom_cube", position_and_color.first,
                   Vec3{1.0f, 1.0f, 1.0f}, eye<float, 3>(), false},
          point_light));
    }
    SSAADepthImage bright_image{width, height, SSAA};
    SSAADepthImage color_image{width, height, SSAA};
    {
      auto white_texture = make_color_texture(Color{1.0f, 1.0f, 1.0f});
      for (auto &[bloom_cube_instance, point_light] :
           bloom_cube_instances_and_point_lights) {
        bloom_generate_light_uv_blinn_phong_renader_homoposition_uv_norm(
            renderer.get(), camera, color_image, model_pool,
            bloom_cube_instance, point_light, white_texture, bright_image);
      }
      {
        float texture_scale = 10.0f;
        auto scale_figure_texture = make_scale_image_texture(
            "../../resources/textures/wood.png", texture_scale);
        auto uv_cube_instance1 =
            Instance{"uv_cube", Vec3{0.0f, -2.0f, 0.0f},
                     Vec3{50.0f, 1.0f, 50.0f}, eye<float, 3>(), false};
        bloom_generate_uv_blinn_phong_renader_homoposition_uv_norm(
            renderer.get(), camera, color_image, model_pool, uv_cube_instance1,
            lights, scale_figure_texture, bright_image);
      }
      {
        auto figure_texture =
            make_image_texture("../../resources/textures/crate-texture.jpg");
        auto uv_cube_instance =
            Instance{"uv_cube", Vec3{0.0f, 1.0f, -4.0f}, Vec3{3.0f, 3.0f, 3.0f},
                     eye<float, 3>(), false};
        bloom_generate_uv_blinn_phong_renader_homoposition_uv_norm(
            renderer.get(), camera, color_image, model_pool, uv_cube_instance,
            lights, figure_texture, bright_image);
      }
      bright_image.hdr_dump("bright_image", 0.5);
      color_image.hdr_dump("color_image", 0.5);
    }
    {
      int blur_times = 10;
       SSAADepthImage temp_bright_image{width, height, SSAA};
      bool is_horizontal = true;
      for(int i = 0; i<blur_times;i++){
          std::cout<<"blur times: "<<i<<"\n";
          if(i%2 == 0){
            bloom_gauss_blur(renderer.get(),temp_bright_image,model_pool,bright_image,is_horizontal); 
          }else{
            bloom_gauss_blur(renderer.get(),bright_image,model_pool,temp_bright_image,is_horizontal); 
          }
          is_horizontal = !is_horizontal;
      }
      bright_image.hdr_dump("blur_image", 0.5);
    }
    bloom_combine(renderer.get(),image,model_pool,color_image,bright_image,0.5); 
    image.dump("xxxx");
  } catch (MyExceptoin &e) {
    std::cout << e.what() << "\n";
  }
