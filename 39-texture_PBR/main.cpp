...
int main() {
  try {
    ...
    lights.push_back(make_pbr_ambient_light());
    lights.push_back(make_pbr_point_light(world_centor + Vec3{0.0f, 0.0f, 5.0f},
                                          Color{300.0f, 300.0f, 300.0f}));
    // lights.push_back(
    //     make_pbr_point_light(world_centor + Vec3{-10.0f, 10.0f, 5.0f},
    //                          Color{300.0f, 300.0f, 300.0f}));
    // lights.push_back(
    //     make_pbr_point_light(world_centor + Vec3{10.0f, 10.0f, 5.0f},
    //                          Color{300.0f, 300.0f, 300.0f}));
    ...
    std::string material_type{"rusted_iron"};
    int scale{1};
    auto albedo_texture = make_scale_image_texture(
        "../../resources/textures/"+material_type+"/albedo.png", scale);
    auto ao_texture = make_scale_image_texture(
        "../../resources/textures/"+material_type+"/ao.png", scale);
    auto metallic_texture = make_scale_image_texture(
        "../../resources/textures/"+material_type+"/metallic.png", scale);
    auto normal_texture = make_scale_image_texture(
        "../../resources/textures/"+material_type+"/normal.png", scale);
    auto roughness_texture = make_scale_image_texture(
        "../../resources/textures/"+material_type+"/roughness.png", scale);
    ...
    for (int row = 0; row < rows; ++row) {
      ...
      for (int col = 0; col < columns; ++col) {
        ...
        pbr_texture_uv_deferred_geometry_rendering_homoposition_uv_norm(
            ...
            uv_sphere_instance, albedo_texture, metallic_texture,
            roughness_texture, ao_texture, normal_texture);
      }
    }
    pbr_uv_deferred_lights_rendering(renderer.get(), camera, image, model_pool,
