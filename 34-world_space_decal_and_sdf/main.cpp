    ...
    */
    {
      ...
      DecalCanvasCoordination decal_canvas_coordination{origin_point, x_axis,
                                                        y_axis, width, height};
      decal_uv_blinn_phong_renader_homoposition_uv_norm(
          renderer.get(), camera, image, model_pool, uv_cube_instance1, lights,
          wood_scale_figure_texture, decal_scale_figure_texture,
          decal_canvas_coordination);
      decal_uv_blinn_phong_renader_homoposition_uv_norm(
          renderer.get(), camera, image, model_pool, uv_cube_instance2, lights,
          crate_figure_texture, decal_scale_figure_texture,
          decal_canvas_coordination);
    }
    {
      auto uv_cube_instance =
          Instance{"uv_cube", Vec3{0.0f, 25.0f, -15.0f},
                   Vec3{50.0f, 50.0f, 1.0f}, eye<float, 3>(), false};
      Vec3 origin_point{0.0, 0.0, 0.0};
      Vec3 x_axis{1.0f, 0.0f, 0.0f};
    ...
    {
      auto cube_center = Vec3{-4.0f, 1.0f, 3.0f};
      auto uv_cube_instance =
          Instance{"uv_cube", cube_center, Vec3{3.0f, 3.0f, 3.0f},
                   eye<float, 3>(), false};
      Color sdf_color{10.0f, 0.0f, 0.0f};
      auto sdf_func = [=](const Vec3 &point) {
        return length(point - cube_center);
      };
      sdf_decal_uv_blinn_phong_renader_homoposition_uv_norm(
          renderer.get(), camera, image, model_pool, uv_cube_instance, lights,
          crate_figure_texture, sdf_func, sdf_color);
    }
    image.dump("xxxx");
