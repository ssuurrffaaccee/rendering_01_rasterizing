  ...
  void render(const std::vector<std::vector<float>> &vertices,
    ...
    for (int i = 0; i < triangle_size; i++) {
      ...
      auto maybe_new_vertices =
          geometry_shader_->process(a_vert, b_vert, c_vert);
      draw_triangle(maybe_new_vertices);
    }
  }
  void draw_triangle(std::vector<VertexStageData> &vertices) {
    for (int i = 2; i < vertices.size(); i += 3) {
      // std::cout<<"cli "<<to_string(vertices[i - 2])<<to_string(vertices[i - 1])<<to_string(vertices[i - 0])<<"\n";
      auto a_vertex = vertex_shader_->process(vertices[i - 2]);
      auto b_vertex = vertex_shader_->process(vertices[i - 1]);
      auto c_vertex = vertex_shader_->process(vertices[i]);
