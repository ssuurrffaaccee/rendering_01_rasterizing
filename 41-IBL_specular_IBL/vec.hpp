...
int64_t clamp_int(int64_t vs, int64_t low, int64_t high) {
  if (vs < low) {
    return low;
  }
  if (vs >= high) {
...
template <typename Type, size_t Len>                  
Vec<Type, Len> pow(const Vec<Type, Len> &r,   
                           const Vec<Type, Len> &l) { 
  Vec<Type, Len> res;                                 
  for (size_t i = 0u; i < Len; i++) {                 
    res[i] = std::pow(r[i],l[i]);                            
  }                                                   
  return res;                                         
}
template <typename Type, size_t Len>
...
std::string to_string(const std::vector<float> &vs) {
  std::stringstream ss;
  ss << "array(";
  for (auto &v : vs) {
    ss << v << ",";
