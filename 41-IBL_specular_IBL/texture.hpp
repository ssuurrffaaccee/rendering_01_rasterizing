...
public:
  ...
  virtual Color sample(float u, float v, float w, int level) { return Color{}; }
};
...
public:
  LevelsTexture(std::vector<std::shared_ptr<CubeMapTexture>> &image_textures) {
    image_textures_ = std::move(image_textures);
    CHECK(image_textures_.size() > 0)
  }
  Color sample(float u, float v, float w, int level) override {
    CHECK(level >= 0 && level < image_textures_.size())
    return image_textures_[level]->sample(u, v, w);
  }
  std::vector<std::shared_ptr<CubeMapTexture>> image_textures_;
};