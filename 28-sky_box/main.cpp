...
int main() {
  try {
    ...
    }
    {
      ...
      auto image_texture =
      make_image_texture("../../resources/textures/brickwall.jpg"); auto
      norm_texture =
      make_image_texture("../../resources/textures/brickwall_normal.jpg");
      // auto figure_texture = make_check_texture();
      auto uv_cube_instance = Instance{"uv_cube", Vec3{0.0f, 0.0f, 0.0f},
                                       Vec3{2.0f, 2.0f, 2.0f}, rotate,
                                       false};
      norm_uv_blinn_phong_renader_homoposition_uv_norm(
          renderer.get(), camera, image, model_pool, uv_cube_instance,
          lights, image_texture, norm_texture);
    }
    image.dump("xxxx");
  } catch (MyExceptoin &e) {
    std::cout << e.what() << "\n";
