...
ModelPool build_model_pool() {
  ...
  ModelPool model_pool;
  {
    ...
    Model uv_point_model;
    {
      float x =1.0f;
      float y =1.0f;
      uv_point_model.points_ = std::vector<Vec3>{
...
int main() {
  try {
    ...
    lights.push_back(make_point_light(world_centor + Vec3{0.0f, 8.0f, 0.0f},
                                      Color{1.0f, 1.0f, 1.0f}));
    auto decal_scale_figure_texture = make_scale_image_texture(
        "../../resources/textures/awesomeface.png", 1.0);
    ...
    // SSAADepthImage source_image{width, height, SSAA};
    ...
    //     renderer.get(), camera, source_image, model_pool, uv_cube_instance,
    //     lights, wood_scale_figure_texture); 
    ...
    auto uv_cube_instance3 =
        Instance{"uv_cube", Vec3{0.0f, 25.0f, -15.0f}, Vec3{50.0f, 50.0f, 1.0f},
                 eye<float, 3>(), false};
    uv_blinn_phong_renader_homoposition_uv_norm(
        renderer.get(), camera, source_image, model_pool, uv_cube_instance1,
        lights, wood_scale_figure_texture);
    uv_blinn_phong_renader_homoposition_uv_norm(
        renderer.get(), camera, source_image, model_pool, uv_cube_instance2,
        lights, crate_figure_texture);
    uv_blinn_phong_renader_homoposition_uv_norm(
        renderer.get(), camera, source_image, model_pool, uv_cube_instance3,
        lights, wood_scale_figure_texture);
    source_image.dump("source");
    SSAADepthImage temp_image0{width, height, SSAA};
    {
      Vec3 origin_point{0.0, 0.0, 0.0};
      Vec3 x_axis{1.0f, 0.0f, 0.0f};
      Vec3 y_axis{0.0f, 0.0f, 1.0f};
      float width{10.0};
      float height{10.0};
      DecalCanvasCoordination decal_canvas_coordination{origin_point, x_axis,
                                                        y_axis, width, height};
      screen_space_decal_uv_blinn_phong_renader_homoposition_uv_norm(
          renderer.get(), camera, temp_image0, model_pool, source_image,
          decal_canvas_coordination, camera_config.near_, camera_config.far_,
          decal_scale_figure_texture);
    }
    temp_image0.rob_depth_buffer(source_image);
    SSAADepthImage temp_image1{width, height, SSAA};
    {
      Vec3 origin_point{0.0, 0.0, 0.0};
      Vec3 x_axis{1.0f, 0.0f, 0.0f};
      ...
      DecalCanvasCoordination decal_canvas_coordination{origin_point, x_axis,
                                                        ...
                                                        height};
      screen_space_decal_uv_blinn_phong_renader_homoposition_uv_norm(
          renderer.get(), camera, temp_image1, model_pool, temp_image0,
          decal_canvas_coordination, camera_config.near_, camera_config.far_,
          decal_scale_figure_texture);
    }
    temp_image1.rob_depth_buffer(temp_image0);
    {
      auto center = Vec3{0.0f, 0.0f, 0.0f};
      Color sdf_color{.5f, 0.0f, 0.0f};
      auto sdf_func = [=](const Vec3 &point) {
        float u = std::abs(point[0]-int(point[0]))-0.5f;
        float v = std::abs(point[2]-int(point[2]))-0.5f;
        return 0.25f - length(Vec2{u,v});
        // return std::abs(int(point[0])%2)+std::abs(int(point[2])%2)-0.9f;
        // return sin(point[0]);
        //return sin(10.0f*length(Vec2{point[0],point[1]}));
        // return sin(10.0f*length(Vec2{float(int(point[0]/0.25f)),float(int(point[2]/0.25f))}));
        // return length(shrink(point) - shrink(center));
      };
      screen_space_sdf_decal_uv_blinn_phong_renader_homoposition_uv_norm(
          renderer.get(), camera, image, model_pool, temp_image1,
          camera_config.near_, camera_config.far_, sdf_func, sdf_color);
    }
    image.dump("xxxx");
