...
enum class LightType {
  ...
  POINT_LIGHT,
  DIRECTIONAL_LIGHT,
  ATTENUATION_POINT_LIGHT,
  AMBIENT_LIGHT
};
class Light {
public:
  Light() { light_type_ = LightType::UNKOWN; }
  virtual ~Light() {}
  ...
  LightType get_type() const { return light_type_; }
...
public:
  PointLight() { light_type_ = LightType::POINT_LIGHT; }
...
public:
  AttenuationPointLight() { light_type_ = LightType::ATTENUATION_POINT_LIGHT; }
  Color shading(const Vec3 &fragment_world_position, const Vec3 &norm,
                const Vec3 &camera_world_position) override {
    auto reverse_view_direction =
        normalize(camera_world_position - fragment_world_position);
...
public:
  DirectionalLight() { light_type_ = LightType::DIRECTIONAL_LIGHT; }
  Color shading(const Vec3 &fragment_world_position, const Vec3 &norm,
                const Vec3 &camera_world_position) override {
    auto reverse_view_direction =
        normalize(camera_world_position - fragment_world_position);
...
public:
  AmbientLight() { light_type_ = LightType::AMBIENT_LIGHT; }
  Color shading(const Vec3 &fragment_world_position, const Vec3 &norm,
                const Vec3 &camera_world_position) override {
    return color_;
  }
