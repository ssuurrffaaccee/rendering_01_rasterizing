...
public:
  ...
  void init(const PerspectiveCameraConfig &config) {
    ...
    build_clipping_planes_in_view_space(config);
  }
  ...
  void
  build_clipping_planes_in_view_space(const PerspectiveCameraConfig &config) {
    // // near
    clipping_planes_in_view_space_.push_back(
        Plane{Vec3{0.0f, 0.0f, 0.0f}, Vec3{0.0f, 0.0f, 1.0f}});
    // far
    // clipping_planes_in_view_space_.push_back(
    //     Plane{Vec3{0.0f,0.0f,config.far_ + 0.001f},
    //           Vec3{0.0f, 0.0f, -1.0f});
    // near plane vector
    Vec3 top_left = Vec3{config.left_, config.top_, config.near_};

    Vec3 top_right = Vec3{config.right_, config.top_, config.near_};
    Vec3 down_left = Vec3{config.left_, config.down_, config.near_};
    Vec3 down_right = Vec3{config.right_, config.down_, config.near_};
    Vec3 origin = Vec3{0.0f, 0.0f, 0.0f}; //- 0.001f * coord_base_.bz;
    // top
    clipping_planes_in_view_space_.push_back(
        Plane{origin, normalize(cross(top_right, top_left))});
    // down
...
public:
  ...
  build_orthogonal_projection_transform(const OrthogonalCameraConfig &config) {
    ...
    Vec3 centor = Vec3{0.5f * (config.right_ + config.left_),
                       0.5f * (config.down_ + config.top_),
                       0.5f * (config.near_ + config.far_)};
    Vec3 scale = Vec3{2.0f / (config.right_ - config.left_),
                      2.0f / (config.top_ - config.down_),
                      2.0f / (config.near_ - config.far_)};
    projection_transform_ = mmdot<float, 4>(
        to_scale_transform(scale), to_position_transform(-1.0f * centor));
  }
  void build_clipping_planes(const OrthogonalCameraConfig &config) {
    // near
    clipping_planes_.push_back(
        Plane{config.position_ + coord_base_.bz * (config.near_ - 0.001f),
              coord_base_.bz});
    ...
    // top
    clipping_planes_.push_back(
        Plane{config.position_ + coord_base_.by * (config.top_ - 0.001f),
              coord_base_.by});
    // down
    clipping_planes_.push_back(
        Plane{config.position_ + coord_base_.by * (config.down_ + 0.001f),
              -1.0f * coord_base_.by});
    // left
    clipping_planes_.push_back(
        Plane{config.position_ + coord_base_.bx * (config.left_ - 0.001f),
              -1.0f * coord_base_.bx});
    // right
    clipping_planes_.push_back(
        Plane{config.position_ + coord_base_.bx * (config.right_ + 0.001f),
              coord_base_.bx});
    // for(auto& plane : clipping_planes_){
    //   std::cout<<to_string(plane.sample_point_)<<to_string(plane.outside_direction_)<<"\n";
    // }
